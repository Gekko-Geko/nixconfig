...
show_upload_speed='false' #"$(tmux_get @tmux_power_show_upload_speed false)"
show_download_speed='false' #"$(tmux_get @tmux_power_show_download_speed false)"
...
user=$(whoami)
LS="#[fg=$G04,bg=$TC,bold] $user_icon $user@#h #[fg=$TC,bg=$G06,nobold]#{?client_prefix,#[bg=$G12],}$right_arrow_icon#[fg=$TC,bg=$G06]#{?client_prefix,#[bg=$G12],} $session_icon #S "
if "$show_upload_speed"; then
    LS="$LS#[fg=$G06,bg=$G05]$right_arrow_icon#[fg=$TC,bg=$G05] $upload_speed_icon #{upload_speed} #[fg=$G05,bg=$BG]$right_arrow_icon"
else
    LS="$LS#[fg=$G06,bg=$BG]#{?client_prefix,#[fg=$G12],}$right_arrow_icon"
fi
